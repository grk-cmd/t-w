...
if(!IS_MAC || samples.length < MIN_SAMPLE){
  ...
  /* ★ A 가 뭉치는데 C 는 가르는 자리 — A 탈락의 **실증**이 될 수 있는 유일한 자리다.
     ⚠️ **[2026-09-15 정정] 이 항목은 판정이 아니라 관찰이다.** 처음에는 `?` 로 두었는데,
       그러면 종료 코드 2 가 되어 **CI 잡이 영구히 빨갛다.**
       그런데 이 값은 **우리 코드와 무관하게 흔들린다** — 러너 이미지에 일렉트론 앱(VS Code 등)이
       깔리는 날 저절로 초록이 된다. 우리가 고칠 수 있는 것이 아닌 것으로 빨강을 만들면
       「늘 켜진 경보」가 되어 잡음이 된다(`CHECKS.md` §13 이 같은 이유로 경고한 자리다).
     ⚠️ 실제로 첫 두 실행에서 이 갈래가 갈렸다:
       1회차 — `siri = Siri.app / Siri.app` 이 basename 충돌로 잡혔다. 서로 다른 번들 id 일
               것이라고 **추정**했고 그 추정이 핸드오프에 실측인 것처럼 적혔다.
       2회차 — 둘은 **같은 번들 id** 였다. 추정이 틀렸다.
       ⇒ 이 표본에는 **A 탈락의 실증이 없다.** A 를 탈락시킨 근거는 여전히 경로 논증뿐이고
         (일렉트론 앱은 실행파일명이 죄다 `Electron`), 그건 6절의 합성 표본으로만 재현된다.
       ★ 합성은 실증이 아니다. **「A 탈락은 논증이지 실측이 아니다」를 이 줄이 계속 말하게 둔다.** */
  const idOf = new Map(rows.map(r => [r.app, r.id]));
  const splitByC = baseDup.filter(([, v]) => new Set(v.map(r => idOf.get(r.app))).size > 1);
  if(baseDup.length === 0){
    say('  · basename 충돌이 이 표본에는 0건 — A 가 옳다는 뜻이 아니라 표본이 그랬다는 뜻이다');
  } else if(splitByC.length === 0){
    say('  · basename 충돌 ' + baseDup.length + '건이 전부 C 에서도 뭉친다 — '
      + '★ 이 표본은 A 탈락의 실증이 아니다(근거는 여전히 경로 논증뿐이다)');
  } else {
...
/* ★★ [2026-09-15] **종료 코드는 「본론이 돌았는가」로 가른다.**
     [처음 규칙] `unknown` 이 하나라도 있으면 2 — `sim-*.js` 의 관례를 그대로 가져온 것이다.
     [무엇이 났나] 4절의 「A 탈락의 실증」 항목이 `?` 를 내는데, 그 값은 **우리 코드와 무관하게**
       러너 이미지에 따라 흔들린다. 그래서 CI 잡이 우리가 고칠 수 없는 이유로 영구히 빨갰다.
       그건 `CHECKS.md` §13 이 경고한 「늘 켜진 경보 → 잡음」 그대로다.
     [지금 규칙] 본론(1절)이 돌았으면 남은 `?` 는 **관찰이지 회귀가 아니다** → 0.
       본론이 못 돌았으면(맥이 아니거나 표본 부족) 아무것도 못 본 것이므로 → 2.
   ⚠️ 실패(`✗`)는 어느 경우에도 1 이다. 이 완화는 **빨강을 줄이지 않는다** —
     빨강과 「못 봤다」를 가르는 선을 옮겼을 뿐이다. 그 구분이 흐려지면 다음 사람이
     `✗` 까지 같이 무르게 푼다. */
const BODY_RAN = IS_MAC && samples.length >= MIN_SAMPLE;
if(unknown) say('  ? 는 이 환경에서 못 본 것이다 — 회귀가 아니다.');
if(fail)    say('  ✗ 는 판정 키 규칙이 실제 맥에서 안 선다는 뜻이다. handoff-mac-runtime.md §7-① 을 볼 것.');
if(!BODY_RAN) say('  ★ 1절(본론)이 안 돌았다 — 이 실행으로는 아무것도 확인되지 않았다.');
if(!fail && BODY_RAN){
  say('  ✓ 빨강 없음 — ★ 그래도 ⑥ 은 안 닫힌다. 클릭 통과·권한·Spaces 는 사람이 필요하다(§7-5).');
}
process.exit(fail ? 1 : (BODY_RAN ? 0 : 2));
